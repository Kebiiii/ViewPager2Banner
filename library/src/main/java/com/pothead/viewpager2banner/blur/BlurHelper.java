package com.pothead.viewpager2banner.blur;
...
public class BlurHelper {
    ...
    public static Bitmap scriptBlur(Context context, Bitmap origin, int outWidth, int outHeight, float radius) {
        ...
        if (blur == null) {
            BannerLogger.e(TAG, "脚本模糊失败，转fastBlur");
        ...
        BannerLogger.i(TAG, "模糊用时：【" + time + "ms】");
        return result;
    }
    ...
    public static Bitmap getViewBitmap(final View v, float scaledRatio, boolean fullScreen) {
        if (v == null || v.getWidth() <= 0 || v.getHeight() <= 0) {
            BannerLogger.e("getViewBitmap  >>  宽或者高为空");
            return null;
        }
        if (statusBarHeight <= 0) statusBarHeight = getStatusBarHeight(v.getContext());
        Bitmap b;
        BannerLogger.i("模糊原始图像分辨率 [" + v.getWidth() + " x " + v.getHeight() + "]");
        ...
        v.draw(c);
        BannerLogger.i("模糊缩放图像分辨率 [" + b.getWidth() + " x " + b.getHeight() + "]");
        return b;
    }
